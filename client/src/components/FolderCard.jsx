...
function FolderCard({ folder, onFolderClick}) {
  ...
  const handleRenameFolder = async (folderId) => {
    ...
    try {
      const response = await fetch(`http://i27.0.0.1:5555/api/folders/${folderId}`, {
        method: "PATCH",
  ...
  const handleFolderDownload = async () => {
    ...
    try {
      const response = await fetch(`http://i27.0.0.1:5555/api/folders/${folder.id}/download`, {
        method: "GET",
        credentials: "include",
      });
  ...
  const confirmMove = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://i27.0.0.1:5555/api/folders/${folder.id}/move`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const handleMoveToTrash = (item) => {
    ...
    fetch(`http://i27.0.0.1:5555/api/folders/${folder.id}/move-to-trash`, {
      method: 'PATCH',
      headers: { "Content-Type": "application/json" },

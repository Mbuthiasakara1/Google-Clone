...
function FileCard({
  ...
  const handleRenameFile = (fileId) => {
    fetch(`https://google-drive-clone-v6g6.onrender.com/api/files/${fileId}`, {
      method: "PATCH",
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await fetch(`https://google-drive-clone-v6g6.onrender.com/api/files/${file.id}/download`, {
        method: 'GET',
        credentials: 'include'
      });
  ...
  const confirmMove = async (item) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://google-drive-clone-v6g6.onrender.com/api/files/${file.id}/move`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
